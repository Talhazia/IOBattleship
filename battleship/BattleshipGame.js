...
function BattleshipGame(id, player1ID, player2ID) {
    this.id = id;
...
BattleshipGame.prototype.abortGame = function(player) {
    ...
    this.gameState = GameState.isGameOver;
...
BattleshipGame.prototype.shoot = function(position) {
    var opponent = this.currentPlayer === 0 ? 1 : 0,
        gridIndex = position.y * Options.gridCols + position.x;

    if(this.players[opponent].shots[gridIndex] === 0 && this.gameState === GameState.isInProgress) {
