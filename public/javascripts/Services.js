var services = angular.module('BattleshipGame.Services', []);
...
services.service('Game', function ($log, Socket, $mdDialog) {
    var GameState = {
        isInProgress: 1,
        isGameOver: 2
    };
    ...
    var miss = new Audio("audio/miss.wav");
    var sunk = new Audio("audio/sunk.wav");

    hit.addEventListener("ended", function(){
        hit.currentTime = 0;
    });

    miss.addEventListener("ended", function(){
        hit.currentTime = 0;
    });

    sunk.addEventListener("ended", function(){
        hit.currentTime = 0;
    });

    var canvas = [], context = [], grid = [],
        gridHeight = 500, gridWidth = 500, gridBorder = 1,
    ...
    function initGame() {
        ...
        gameStatus = GameState.isInProgress;
    ...
    function setTurn(turnState) {
        if(gameStatus !== GameState.isGameOver) {
            turn = turnState;
    ...
    function drawSquares(gridIndex) {
        ...
        for(i = 0; i < gridRows; i++) {
            for(j = 0; j < gridCols; j++) {
                ...
                context[gridIndex].fillStyle = '#3498db';
        ...
        for(i = 0; i < gridRows; i++) {
            for(j = 0; j < gridCols; j++) {
                ...
                if(grid[gridIndex].shots[i * gridCols + j] === 1) {
                    ...
                    context[gridIndex].strokeStyle = '#c0392b';
                ...
                // draw red circle if hit on square
                    ...
                    if(isImgLoaded) {
                        //hit.play();
                        context[gridIndex].drawImage(img, squareX + 5, squareY + 5, 40, 40);
                    }
                }
            }
        }
